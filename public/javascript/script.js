...
function getQuiz(){
    ...
    var amt = document.getElementsByName("trivia_amount")[0].value;
    var cat = document.getElementsByName("trivia_category")[0].value;
    var dif = document.getElementsByName("trivia_difficulty")[0].value;
    var typ = document.getElementsByName("trivia_type")[0].value
    var searchstring = ""
    if (amt > 0) {
        searchstring = "amount=" + amt;
        if (cat != "any") {
            searchstring += "&category=" + cat;
            if (dif != "any") {
                searchstring += "&difficulty=" + dif;
                if (typ != "any") {
                    searchstring += "&multiple=" + typ;
                }
            }
        }
    }
    ...
    fetch(
        ...
        .then((loadedQuestions) => {
            questions = loadedQuestions.results.map((loadedQuestion) => {
                const formattedQuestion = {
                    question_string: loadedQuestion.question,
                    choices: {
                        correct: loadedQuestion.correct_answer,
                        wrong: loadedQuestion.incorrect_answers,
                    }
                };        
                // console.log(formattedQuestion)
                qStore.push(formattedQuestion)
                all_questions = [formattedQuestion];
            });
            console.log(qStore)
            startQuiz(qStore) 
        })
...
function startQuiz(all_questions) {
